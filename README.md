...
**Please Note:** This `README.md` contains mathematical equations that are best viewed on the dedicated project documentation site, which fully supports LaTeX rendering via MathJax.

**[Click here to view the full documentation with correctly rendered equations.](https://yourusername.github.io/your-repo-name/)**
*(Replace `https://yourusername.github.io/your-repo-name/` with the actual URL of your GitHub Pages site)*
...
* **Calculations**:
    ...
    * Calculates the second derivatives of all states (linear accelerations and angular accelerations) based on the forces, moments of inertia, drag coefficients, and disturbances. The dynamic model of a UAV is given by equation (1) in the paper:
        `$$\ddot{x}=(\sin\phi\sin\psi+\cos\phi\sin\theta\cos\psi)\frac{U_{1}}{m}-\frac{k_{1}}{m}\dot{x},$$`
        `$$\ddot{y}=(-\sin\phi\cos\psi+\cos\phi\sin\theta\sin\psi)\frac{U_{1}}{m}-\frac{k_{2}}{m}\dot{y}$$`
        `$$\ddot{z}=-g+(\cos\phi\cos\theta)\frac{U_{1}}{m}-\frac{k_{3}}{m}\dot{z},$$`
